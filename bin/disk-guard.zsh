...
#
# Thresholds via env (see lib/common.zsh): PL_WARN_PCT (15), PL_CRIT_PCT (10).
...
local STATE=$PL_STATE_DIR/guard.state
...
#
# This used to sit below pl_run_health_checks and only launch the refresh, so
...
#
# ORDER IS THE POINT: pl_run_health_checks builds the Backup row's headline and
...
if pl_sizes_stale; then
  ( nice -n 15 zsh -c "source ${0:A:h}/lib/common.zsh; pl_sizes_refresh" >/dev/null 2>&1 & ) &!
fi

local level=$(pl_overall_level) rc=0
...
# with 121 GB spare.
local worst=$(pl_worst_check)
local subject=$(pl_check_name $worst)
local headline=$(pl_check_headline $worst)
...
for c in $PL_CHECKS; do
  ...
  [[ $lvl == OK ]] && continue
  lines+=("$(pl_check_detail $c)")
done
local msg="${(j: :)lines}"
[[ -z $msg ]] && msg="$(pl_check_detail $worst)"
(( ${#PL_NOTES} )) && msg="$msg  [${(j:; :)PL_NOTES}]"
...
local log_pct=$(pl_pct_free) log_free=$(pl_free_bytes)
pl_log "guard $level pct=${log_pct:-?} free=${log_free:-?} subject=$subject headline=\"$headline\" notes=${(j:,:)PL_NOTES}"

# ---- de-dup: renotify only on escalation, or once per PL_RENOTIFY_H hours ---
...
elif [[ $level != OK ]] && (( now - prev_notify_ts > PL_RENOTIFY_H * 3600 )); then should_notify=1
fi
...
if (( json )); then
  ...
  for c in $PL_CHECKS; do
    ...
    printf '{"level":"%s","name":"%s","headline":"%s","detail":"%s"}' \
      "$(pl_check_level $c)" "$(pl_check_name $c)" "$(pl_check_headline $c)" \
      "$(pl_check_detail $c | sed 's/"/\\"/g')"
  done
  printf '],"notes":['
  first=1
  for c in $PL_NOTES; do
    (( first )) || printf ','
    first=0
    printf '"%s"' "${c//\"/\\\"}"
  done
...
# ---- output -----------------------------------------------------------------
case $level in
  (CRIT) pl_crit "${subject}: ${headline}" ;;
  (WARN) pl_warn "${subject}: ${headline}" ;;
  (OK)   pl_ok   "all checks pass" ;;
esac
for c in $PL_CHECKS; do
  [[ $(pl_check_level $c) == OK ]] && continue
  pl_info "$(pl_check_detail $c)"
done
...
if (( should_notify && ! quiet )); then
  ...
  if (( $+commands[terminal-notifier] )); then
    terminal-notifier -title "$title" -message "$msg" -group plimsoll 2>/dev/null
  else
